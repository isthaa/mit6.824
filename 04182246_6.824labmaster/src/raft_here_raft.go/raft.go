...
ApplyMsg
每次向日志提交新条目时，每个Raft对等体都应该向同一服务器中的服务(或测试器)发送ApplyMsg
*/
...
/*
当每个Raft节点意识到连续的日志条目已经被提交时，节点应该通过传递给Make()的applyCh向同一服务器上的服务（或测试器）发送一个ApplyMsg。
...
如果此服务器不是领导者，则返回false。否则开始达成一致并立即返回。不能保证这个命令将被提交到Raft日志中，
因为领导者可能会失败或输掉选举。即使Raft实例已被终止，此函数也应正常返回。

第一个返回值是命令将出现的索引（如果它被提交）。第二个返回值是当前term。如果此服务器认为它是
领导者，则第三个返回值为true。
*/
...
func Make(peers []*labrpc.ClientEnd, me int,persister *Persister, applyCh chan ApplyMsg) *Raft {
	...
	// initialize from state persisted before a crash(从崩溃前的状态进行初始化)
	rf.readPersist(persister.ReadRaftState())
	// initialize commitIndex after recover(恢复后初始化commitIndex)
	rf.commitIdx = rf.getFirstIndex()

	// start ticker goroutine to check Election and appendEntry(启动ticker例程来检查选举并追加条目)
